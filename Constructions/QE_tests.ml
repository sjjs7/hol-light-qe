...
(* We want to ensures mk_not_effective_in creates the right formula, that the NEI formulas must be explicitly added to the NEI list, 
...
let axiom_A4_6 = prove(mk_not_effective_in `x:aty` `(\x:aty. P:aty)` `y:aty`, 
...
let bound_by_quote = prove(mk_not_effective_in `x:epsilon` `Q_ (x:epsilon) _Q` `y:epsilon`,
							GEN_TAC THEN CONV_TAC(RATOR_CONV(RAND_CONV(BETA_CONV))) THEN REFL_TAC);;
